...
interface ValidationState {
  ...
  buildBaselineFailure: string | null;
...
export class CodexExecutionService implements ExecutionProvider {
  ...
  private async runValidationGate(input: {
    ...
    if (input.validationState.build.available && !input.validationState.build.passed) {
      ...
      let buildResult = await this.runValidationCommand(input.snapshot.repoDir, buildCommand);
      input.testsRun.push(buildCommand);

      if (!buildResult.ok) {
        const repaired = await this.attemptBuildEnvironmentRepair({
          repoDir: input.snapshot.repoDir,
          packageManager: input.validationState.packageManager,
          buildOutput: buildResult.output,
          testsRun: input.testsRun,
        });

        if (repaired) {
          buildResult = await this.runValidationCommand(input.snapshot.repoDir, buildCommand);
          input.testsRun.push(`${buildCommand} (post-repair)`);
        }
      }
      ...
      if (buildResult.ok) {
        ...
        input.validationState.buildBaselineFailure &&
        this.isSameBuildFailure(input.validationState.buildBaselineFailure, buildResult.output)
      ) {
        input.validationState.build.passed = true;
        input.validationState.validationWarnings.push(
          'Build validation still fails with the same baseline error that existed before the AI change.',
        );
  ...
  private async finalizeJob(input: {
    ...
    const validationWarningText = input.validationState.validationWarnings.length
      ? `Validation warnings: ${input.validationState.validationWarnings.join(' | ')}`
      : null;
    ...
    await this.prisma.agentJob.update({
      ...
      data: {
        ...
        pullRequestUrl,
        summary:
          [input.finish.summary, validationWarningText]
            .filter(Boolean)
            .join('\n') ||
  ...
  private async captureBuildBaseline(
    repoDir: string,
    validationState: ValidationState,
  ): Promise<void> {
    if (!validationState.build.available) {
      return;
    }

    const buildCommand = this.getValidationCommand(validationState.packageManager, 'build');
    let baseline = await this.runValidationCommand(repoDir, buildCommand);

    for (let repairAttempt = 0; repairAttempt < 3 && !baseline.ok; repairAttempt += 1) {
      const repaired = await this.attemptBuildEnvironmentRepair({
        repoDir,
        packageManager: validationState.packageManager,
        buildOutput: baseline.output,
        testsRun: [],
      });
      ...
      baseline = await this.runValidationCommand(repoDir, buildCommand);
    }

    if (!baseline.ok) {
      validationState.buildBaselineFailure = baseline.output;
      this.logger.warn('Baseline build failed before AI edits; validation will compare against it.');
    }
  }

  private isSameBuildFailure(baseline: string, current: string): boolean {
    const normalize = (value: string) =>
      value
        .replace(/\/tmp\/ai-pr-codex-[^/\s]+/g, '/tmp/ai-pr-codex-<workspace>')
        .replace(/\s+/g, ' ')
        .trim()
        .toLowerCase();
    ...
    ].filter((signal) => normalizedBaseline.includes(signal));

    return baselineSignals.length > 0 && baselineSignals.every((signal) => normalizedCurrent.includes(signal));
  }

  private async attemptBuildEnvironmentRepair(input: {
    repoDir: string;
    packageManager: 'npm' | 'pnpm' | 'yarn';
    buildOutput: string;
    testsRun: string[];
  }): Promise<boolean> {
    const normalizedOutput = input.buildOutput.toLowerCase();
    const [browserslistCommand, reinstallCommand, babelPluginCommand] =
      this.executionWorkspaceService.getValidationRepairCommands(input.packageManager);
    const repairCommands: string[] = [];

    if (normalizedOutput.includes('browserslist: caniuse-lite is outdated')) {
      repairCommands.push(browserslistCommand);
    }

    if (
      normalizedOutput.includes('react-scripts: not found') ||
      normalizedOutput.includes("'react-scripts' is not recognized")
    ) {
      repairCommands.push(reinstallCommand);
    }

    if (
      normalizedOutput.includes("cannot find module '@babel/plugin-proposal-private-property-in-object'") ||
      normalizedOutput.includes('cannot find module "@babel/plugin-proposal-private-property-in-object"')
    ) {
      repairCommands.push(babelPluginCommand);
    }
    ...
    for (const command of repairCommands) {
      const result = await this.runValidationCommand(input.repoDir, command);
      input.testsRun.push(command);
